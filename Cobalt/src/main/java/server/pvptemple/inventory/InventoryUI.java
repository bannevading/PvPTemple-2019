...
public class InventoryUI {
   ...
   @Getter
   private final String title;@Getter
   ...
   public Inventory2D getCurrentUI() {
      return (Inventory2D)this.inventories.get(this.page);
   }
   ...
   public ClickableItem getItem(int slot) {
      ...
      Inventory2D lastInventory = (Inventory2D)this.inventories.get(this.inventories.size() - 1);
   ...
   public void removeItem(int slot) {
      Inventory2D inventory2D = (Inventory2D)this.inventories.get(this.page);
      this.setItem(slot, (ClickableItem)null);

      for(int i = slot + 1; i < this.getSize(); ++i) {
         ClickableItem item = this.getItem(i);
         this.setItem(i - 1, item);
         this.setItem(i, (ClickableItem)null);
      }
   ...
   public class Inventory2D {
      ...
      public Inventory toInventory() {
         if (this.cachedInventory != null) {
            ...
            Inventory inventory = CorePlugin.getInstance().getServer().createInventory(new InventoryUIHolder(InventoryUI.this, this.title, this.rows * 9), this.rows * 9, this.title);
