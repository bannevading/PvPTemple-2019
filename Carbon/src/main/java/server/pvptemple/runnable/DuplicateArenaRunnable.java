...
public abstract class DuplicateArenaRunnable extends BukkitRunnable {
   ...
   public DuplicateArenaRunnable(Carbon plugin, Arena copiedArena, int offsetX, int offsetZ, int incrementX, int incrementZ) {
      this.plugin = plugin;
   ...
   public void run() {
      if (this.paste == null) {
         ...
         for(Location loc : copy.keySet()) {
            if (((Block)copy.get(loc)).getType() != Material.AIR) {
               this.paste.put(loc.clone().add((double)this.offsetX, (double)0.0F, (double)this.offsetZ), copy.get(loc));
            }
         }
         ...
         for(Location loc : this.paste.keySet()) {
            if (((Block)this.paste.get(loc)).getType() != Material.AIR) {
               newPaste.put(loc.clone().add((double)this.incrementX, (double)0.0F, (double)this.incrementZ), this.paste.get(loc));
            }
         }
