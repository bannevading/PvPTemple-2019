      ...
      if (!this.blocks.isEmpty() && this.iterator.hasNext()) {
         TaskManager.IMP.async(() -> {
            ...
            for(Map.Entry<Location, Block> entry : this.blocks.entrySet()) {
               try {
                  editSession.setBlock(new Vector((double)((Location)entry.getKey()).getBlockX(), (double)((Location)entry.getKey()).getBlockY(), ((Location)entry.getKey()).getZ()), new BaseBlock(((Block)entry.getValue()).getTypeId(), ((Block)entry.getValue()).getData()));
               } catch (Exception var5) {
               }
            }

            editSession.flushQueue();
            ConcurrentMap var10001 = this.blocks;
            TaskManager.IMP.task(var10001::clear);
         });
      } else {
         this.finish();
         this.completed = true;
